...
- Non-demo auth has been smoke-tested locally end-to-end with `AUTH_DEV_LOG_MAGIC_LINK=true` (see `docs/non-demo-auth-smoke.md`). Production sign-in uses the Auth.js Resend provider via `AUTH_RESEND_KEY` + `AUTH_EMAIL_FROM`; SMTP via `EMAIL_SERVER` + `EMAIL_FROM` is kept as a legacy fallback. There is no production deployment of non-demo mode yet.
