...
export async function POST(request: Request, { params }: { params: Promise<{ requestId: string }> }) {
  try {
    ...
    const updated = await transitionGroceryRequest(requestId, actorId, GroceryRequestStatus.REJECTED, "GROCERY_REQUEST_REJECTED");
    return NextResponse.json({ request: updated });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Could not reject grocery request." }, { status: error instanceof PermissionError ? 403 : 400 });
  }
}
