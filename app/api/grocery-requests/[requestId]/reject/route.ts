...
export async function POST(request: Request, { params }: { params: Promise<{ requestId: string }> }) {
  try {
    ...
    const updated = await transitionGroceryRequest(requestId, actor.actorId, GroceryRequestStatus.REJECTED, "GROCERY_REQUEST_REJECTED");
    return NextResponse.json({ request: updated });
  } catch (error) {
    return apiErrorResponse(error, "Could not reject grocery request.");
  }
}
