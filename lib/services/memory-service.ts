  ...
  for (const [canonicalName, itemEvents] of groupedEvents) {
    ...
    if (medianInterval && daysSinceLast >= Math.max(1, medianInterval - 1)) {
      dueSoon.push({
        ...
        reason: `${latest.displayName} — usually expected every ${formatInterval(medianInterval)}. Last seen ${formatDaysAgo(daysSinceLast)}.`,
        confidence,
    ...
    if (medianInterval && monthlyCategories.has(latest.category) && medianInterval >= 21 && medianInterval <= 45) {
      monthlyStaples.push({
        ...
        displayName: latest.displayName,
        category: latest.category,
        reason: `${latest.displayName} — recurs roughly monthly in this household's history${quantity ? `. Add ${formatQuantity(quantity, unit)}?` : "."}`,
        confidence,
        lastSeenAt: latest.seenAt,
        suggestedQuantity: quantity,
        suggestedUnit: unit,
    ...
    if (sorted.length >= 3) {
      frequentItems.push({
        ...
        displayName: latest.displayName,
        category: latest.category,
        reason: `${latest.displayName} — seen ${sorted.length} times in household grocery activity.`,
        confidence,
        lastSeenAt: latest.seenAt,
        suggestedQuantity: quantity,
        suggestedUnit: unit,
...
function suggestionFromPattern(pattern: RecurringPatternInput, now: Date): MemorySuggestion | null {
  ...
  return {
    ...
    reason: `${pattern.groceryItem.displayName} — usually expected every ${formatInterval(pattern.averageIntervalDays)}. Last seen ${formatDaysAgo(daysSinceLast)}.`,
...
function buildSetupSuggestions(): MemorySuggestion[] {
  return groceryCatalogSeeds
    ...
    .map((item) => ({
      ...
      reason: `Setup suggestion: ${item.displayName} — commonly used in many households. Add it once to help HomeStock learn your actual pattern.`,
      confidence: 0.35,
      lastSeenAt: null,
