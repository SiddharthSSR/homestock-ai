...
export function PageHeader({ eyebrow = "Household", title, description, meta, className = "", children }: PageHeaderProps) {
  return (
    <section className={`w-full min-w-0 flex-1 overflow-hidden rounded-[2rem] border border-cocoa/10 bg-paper shadow-editorial ${className}`}>
      <div className="grid gap-6 p-6 sm:p-8 md:grid-cols-[1fr_auto] md:items-end">
        <div>
          ...
          <h1 className="font-editorial mt-3 max-w-3xl break-words text-4xl font-semibold leading-[0.98] text-cocoa sm:text-5xl md:text-6xl">{title}</h1>
